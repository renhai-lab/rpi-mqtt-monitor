...
if config.ext_sensors:
    ...
    sys.path.append(
        os.path.abspath(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "..", "ext_sensor_lib"
            )
        )
    )
    import ds18b20
...
configlanguage.read(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "translations.ini")
)
...
def check_wifi_signal(format):
    try:
        ...
        interface = (
            subprocess.Popen(full_cmd, shell=True, stdout=subprocess.PIPE)
            .communicate()[0]
            .strip()
            .decode("utf-8")
        )
        full_cmd = "/sbin/iwconfig {} | grep -i quality".format(interface)
        wifi_signal = subprocess.Popen(
            full_cmd, shell=True, stdout=subprocess.PIPE
        ).communicate()[0]

        if format == "dbm":
            wifi_signal = (
                wifi_signal.decode("utf-8").strip().split(" ")[4].split("=")[1]
            )
        else:
            wifi_signal = (
                wifi_signal.decode("utf-8")
                .strip()
                .split(" ")[1]
                .split("=")[1]
                .split("/")[0]
...
def check_swap():
    ...
    swap = subprocess.Popen(full_cmd, shell=True, stdout=subprocess.PIPE).communicate()[
        0
    ]
...
def check_uptime(format):
    if format == "timestamp":
        ...
        tz_cmd = "date +%z"
        tz_str = (
            subprocess.Popen(tz_cmd, shell=True, stdout=subprocess.PIPE)
            .communicate()[0]
            .decode("utf-8")
            .strip()
        )
        ...
        iso_timestamp = (
            timestamp.isoformat() + tz_str
        )  # Append correct offset to indicate `local` time
        return iso_timestamp
    else:
        full_cmd = "awk '{print int($1" + format + ")}' /proc/uptime"

    return int(
        subprocess.Popen(full_cmd, shell=True, stdout=subprocess.PIPE).communicate()[0]
    )
...
def check_git_version(script_dir):
    full_cmd = "/usr/bin/git -C {} describe --tags `/usr/bin/git -C {} rev-list --tags --max-count=1`".format(
...
def check_all_drive_temps():
    ...
    for hwmon in hwmon_devices:
        ...
        if device_name and any(
            keyword in device_name.lower() for keyword in ["nvme", "sd"]
        ):
...
def check_disk_status(mount_path):
    """
    Check if a disk is mounted at the specified path.
    Returns True if mounted, False if not, None on error.
    """
    try:
        ...
        print(f"Error checking disk mount status for {mount_path}: {e}")
        return None if config.use_availability else False


def check_disk_usage(mount_path):
    """
    Get disk usage information for a mounted disk.
    Returns dict with total_gb, free_gb, and used_percent.
    Returns None for all values if disk is not mounted or on error.
    """
    try:
        if not check_disk_status(mount_path):
        ...
        print(f"Error getting disk usage for {mount_path}: {e}")
        return {"total_gb": None, "free_gb": None, "used_percent": None}
...
:: Release notes {}:
{}""".format(
        os.path.dirname(script_dir), remote_version, get_release_notes(remote_version)
    )
    print(output)
...
def get_release_notes(version):
    ...
    try:
        ...
        release_notes = (
            None if config.use_availability else "No release notes available"
        )
...
def add_common_attributes(
    data, icon, name, unit=None, device_class=None, state_class=None
):
    data.update(
        {
            "icon": icon,
...
def handle_specific_configurations(data, what_config, device):
    if what_config == "cpu_load":
        ...
        add_common_attributes(
            data,
            "hass:thermometer",
            get_translation("cpu_temperature"),
            "°C",
            "temperature",
            "measurement",
        )
        ...
        add_common_attributes(
            data, "mdi:harddisk", get_translation("disk_swap"), "%", None, "measurement"
        )
    elif what_config == "memory":
        add_common_attributes(
            data,
            "mdi:memory",
            get_translation("memory_usage"),
            "%",
            None,
            "measurement",
        )
        ...
        add_common_attributes(
            data,
            "mdi:speedometer",
            get_translation("cpu_clock_speed"),
            "MHz",
            "frequency",
            "measurement",
        )
        ...
        add_common_attributes(
            data, "mdi:wifi", get_translation("wifi_signal"), "%", None, "measurement"
        )
    elif what_config == "wifi_signal_dbm":
        add_common_attributes(
            data,
            "mdi:wifi",
            get_translation("wifi_signal_strength"),
            "dBm",
            "signal_strength",
            "measurement",
        )
        ...
        add_common_attributes(
            data, "mdi:fan", get_translation("fan_speed"), "RPM", None, "measurement"
        )
        ...
        add_common_attributes(
            data,
            "mdi:git",
            get_translation("rpi_mqtt_monitor"),
            None,
            "update",
            "measurement",
        )
        ...
        data["value_template"] = (
            "{{ 'ON' if value_json.installed_ver != value_json.new_ver else 'OFF' }}"
        )
    elif what_config == "update":
        version = update.check_git_version_remote(script_dir)
        add_common_attributes(
            data, "mdi:update", get_translation("rpi_mqtt_monitor"), None, "firmware"
        )
        data["title"] = "New Version"
        data["state_topic"] = (
            config.mqtt_uns_structure
        ...
        data["value_template"] = (
            "{{ {'installed_version': value_json.installed_ver, 'latest_version': value_json.new_ver } | to_json }}"
        )
        data["command_topic"] = (
            config.mqtt_discovery_prefix + "/update/" + hostname + "/command"
        )
        data["payload_install"] = "install"
        data["release_url"] = (
            "https://github.com/hjelev/rpi-mqtt-monitor/releases/tag/" + version
        )
        data["entity_picture"] = (
            "https://raw.githubusercontent.com/hjelev/rpi-mqtt-monitor/refs/heads/master/images/update_icon.png"
        )
        ...
        add_common_attributes(
            data,
            "hass:thermometer",
            device + " " + get_translation("temperature"),
            "°C",
            "temperature",
            "measurement",
        )
        ...
        add_common_attributes(
            data,
            "mdi:water-percent",
            device + " " + get_translation("humidity"),
            "%",
            "humidity",
            "measurement",
        )
        ...
        add_common_attributes(
            data, "mdi:upload", get_translation("data_sent"), "MB", None, "measurement"
        )
    elif what_config == "data_received":
        add_common_attributes(
            data,
            "mdi:download",
            get_translation("data_received"),
            "MB",
            None,
            "measurement",
        )
    elif what_config.startswith("disk_") and "_mounted" in what_config:
        # Extract disk name from config key like "disk_backup_1t_mounted"
        disk_name = what_config.replace("disk_", "").replace("_mounted", "")
        add_common_attributes(
            data, "mdi:harddisk", f"{disk_name.replace('_', ' ').title()} Mounted"
        )
        data["device_class"] = "connectivity"
        ...
        add_common_attributes(
            data,
            "mdi:harddisk",
            f"{disk_name.replace('_', ' ').title()} Total Size",
            "GB",
            "data_size",
            "measurement",
        )
    elif what_config.startswith("disk_") and "_free" in what_config:
        disk_name = what_config.replace("disk_", "").replace("_free", "")
        add_common_attributes(
            data,
            "mdi:harddisk",
            f"{disk_name.replace('_', ' ').title()} Free Space",
            "GB",
            "data_size",
            "measurement",
        )
    elif what_config.startswith("disk_") and "_used_percent" in what_config:
        disk_name = what_config.replace("disk_", "").replace("_used_percent", "")
        add_common_attributes(
            data,
            "mdi:chart-donut",
            f"{disk_name.replace('_', ' ').title()} Used",
            "%",
            None,
            "measurement",
        )
...
def config_json(what_config, device="0", hass_api=False):
    ...
    if (
        "state_class" in data
        and data["state_class"] == "measurement"
        and what_config != "git_update"
    ):
...
def create_mqtt_client():
    ...
    client = paho.Client(
        client_id="rpi-mqtt-monitor-" + hostname + str(int(time.time()))
    )
...
def publish_update_status_to_mqtt(git_update, apt_updates):
    ...
    if config.update:
        if config.discovery_messages:
            client.publish(
                config.mqtt_discovery_prefix + "/update/" + hostname + "/config",
                config_json("update"),
                qos=1,
            )
...
def publish_to_mqtt(monitored_values):
    ...
    for key, value in monitored_values.items():
        if (
            key not in non_standard_values
            and key in config.__dict__
            and config.__dict__[key]
        ):
            if config.discovery_messages:
                client.publish(
            ...
            if config.use_availability:
                client.publish(
                    ...
                    "offline" if value is None else "online",
                    qos=config.qos,
                )
            client.publish(
            ...
            if item[1] == "sht21":
                if config.discovery_messages:
                    ...
                    client.publish(
                        ...
                        + item[0]
                        + "_temp_status/config",
                        config_json("sht21_temp_status", device=item[0]),
                        qos=config.qos,
                    )
                    # humidity
                    client.publish(
    ...
    if "data_sent" in monitored_values:
        if config.discovery_messages:
            client.publish(
                ...
                config_json("data_sent"),
                qos=config.qos,
            )
        if config.use_availability:
            client.publish(
                f"{config.mqtt_uns_structure}{config.mqtt_topic_prefix}/{hostname}/data_sent_availability",
                "offline" if monitored_values["data_sent"] is None else "online",
                qos=config.qos,
            )
        client.publish(
            f"{config.mqtt_uns_structure}{config.mqtt_topic_prefix}/{hostname}/data_sent",
            monitored_values["data_sent"],
            qos=config.qos,
            retain=config.retain,
        )
    ...
    if "data_received" in monitored_values:
        if config.discovery_messages:
            client.publish(
                ...
                config_json("data_received"),
                qos=config.qos,
            )
        if config.use_availability:
            client.publish(
                f"{config.mqtt_uns_structure}{config.mqtt_topic_prefix}/{hostname}/data_received_availability",
                "offline" if monitored_values["data_received"] is None else "online",
                qos=config.qos,
            )
        client.publish(
            f"{config.mqtt_uns_structure}{config.mqtt_topic_prefix}/{hostname}/data_received",
            monitored_values["data_received"],
            qos=config.qos,
            retain=config.retain,
        )

    # Publish disk monitoring sensors
    if (
        hasattr(config, "disk_monitoring")
        and config.disk_monitoring
        and hasattr(config, "disk_mounts")
    ):
        for key in monitored_values.keys():
            if key.startswith("disk_"):
                ...
                if config.discovery_messages:
                    client.publish(
                        f"{config.mqtt_discovery_prefix}/{sensor_type}/{config.mqtt_topic_prefix}/{hostname}_{key}/config",
                        config_json(key),
                        qos=config.qos,
                    )
                if config.use_availability:
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        "--hass_api",
        action="store_true",
        help="send readings via Home Assistant API (not via MQTT)",
        default=False,
    )
    parser.add_argument(
        "-d",
    ...
    parser.add_argument(
        ...
        "--service",
        action="store_true",
        help="run script as a service, sleep interval is configurable in config.py",
        default=False,
    )
    parser.add_argument(
        "-v",
        "--version",
        action="store_true",
        help="display installed version and exit",
        default=False,
    )
    parser.add_argument(
        "-u",
        "--update",
        action="store_true",
        help="update script and config then exit",
        default=False,
    )
    parser.add_argument(
        "-w",
        "--hass_wake",
        action="store_true",
        help="display Home assistant wake on lan configuration",
        default=False,
    )
    parser.add_argument(
        "--uninstall",
        action="store_true",
        help="uninstall rpi-mqtt-monitor and remove all related files",
    )
...
def collect_monitored_values():
    ...
    if (
        ...
        for disk_name, mount_path in config.disk_mounts:
            # Check mount status
            ...
            monitored_values[f"disk_{disk_name}_total"] = usage["total_gb"]
            monitored_values[f"disk_{disk_name}_free"] = usage["free_gb"]
            monitored_values[f"disk_{disk_name}_used_percent"] = usage["used_percent"]
    ...
    while not stop_event.is_set():
        ...
        if args.hass_api:
            if (
                ...
                and config.hass_token != "your_hass_token"
            ):
...
def uninstall_script():
    ...
    script_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "../remote_install.sh"
    )
...
def on_message(client, userdata, msg):
    ...
    if msg.payload.decode() == "install":
        ...
        os.system(
            'su -l {} -c "xset -display :0 dpms force off"'.format(config.os_user)
        )
